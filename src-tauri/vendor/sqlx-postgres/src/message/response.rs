...
impl TryFrom<&str> for PgSeverity {
    ...
    fn try_from(s: &str) -> Result<PgSeverity, Error> {
        let result = match s {
            ...
            "LOG" => PgSeverity::Log,
            // Some PostgreSQL-wire-compatible databases localize both the S
            // and V severity fields even though PostgreSQL specifies that V
            // must stay in English. Accept the common Chinese values so the
            // SQLSTATE and actual database message remain available.
            "严重错误" | "致命错误" => PgSeverity::Fatal,
            "错误" => PgSeverity::Error,
            "警告" => PgSeverity::Warning,
            "通知" => PgSeverity::Notice,
            "调试" => PgSeverity::Debug,
            "信息" => PgSeverity::Info,
            "日志" => PgSeverity::Log,
    ...
    fn decode_with(buf: Bytes, _: ()) -> Result<Self, Error> {
        ...
        for (field, v) in fields {
            ...
            match field {
                ...
                b'V' => {
                    // PostgreSQL promises that V is not localized, but some
                    // protocol-compatible databases violate that contract.
                    // Unknown text must not hide the SQLSTATE and message.
                    severity_v = from_utf8(&buf[v.clone()])
                        .map_err(|_| notice_protocol_err())?
                        .try_into()
                        .ok();
                }
...
#[test]
fn test_decode_localized_severity_from_pg_compatible_server() {
    const DATA: &[u8] = "S错误\0V错误\0C42703\0M字段 id_med_alias 不存在\0\0".as_bytes();

    let message = Notice::decode(Bytes::from_static(DATA)).unwrap();

    assert_eq!(message.severity(), PgSeverity::Error);
    assert_eq!(message.code(), "42703");
    assert_eq!(message.message(), "字段 id_med_alias 不存在");
}

#[test]
fn test_unknown_compatible_severity_keeps_error_details() {
    const DATA: &[u8] = "S数据库错误\0V数据库错误\0C42501\0M当前用户没有查询权限\0\0".as_bytes();

    let message = Notice::decode(Bytes::from_static(DATA)).unwrap();

    assert_eq!(message.severity(), PgSeverity::Log);
    assert_eq!(message.code(), "42501");
    assert_eq!(message.message(), "当前用户没有查询权限");
}
