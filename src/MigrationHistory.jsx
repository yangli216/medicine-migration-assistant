...
import { statusMeta, SummaryCards } from "./MigrationStatus";

export function migrationTaskLabel(sourceType = "") {
  return (
    {
      PHIS27: "二系列phis药品基础数据",
      INSTITUTION_INVENTORY: "机构库存首次盘点",
      PHIS27_INVENTORY: "机构库存首次盘点",
      CSV: "文件导入",
      JSON: "文件导入",
      DATABASE: "数据库导入",
...
function historyBatchTitle(sourceType = "") {
  if (["INSTITUTION_INVENTORY", "PHIS27_INVENTORY"].includes(sourceType))
    return "机构库存首次盘点";
...
function historyBatchScope(batch) {
  ...
  if (["INSTITUTION_INVENTORY", "PHIS27_INVENTORY"].includes(batch.sourceType)) {
